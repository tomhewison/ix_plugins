...
hook.Add("LoadFonts", "ixOutfitPororiFont", function(font, genericFont)
	surface.CreateFont("ixOutfitFont", {
		font = font,
		extended = true,
		size = ScreenScale(30),
		weight = 1000
...
vgui.Register("ixOutfitModel", PANEL, "DModelPanel")
...
local PANEL = {}
	...
	function PANEL:setupTitle(ab)
		function self.title:Paint(w, h) --(text, x, y, color, alignX, alignY, font, alpha)
			ix.util.DrawText(L"Closet", w / 2, h / 2, color_white, 1, 1, "ixOutfitFont")
		end
	end
	...
	function PANEL:setupBottom(ab, ac)
		...
		self.close:SetFont("ixMediumFont")
		...
		self.apply:SetFont("ixMediumFont")
		...
		self.apply.DoClick = function()
			...
			netstream.Start("ixApplyOutfit", outfits)
		end
	end
	...
	function PANEL:setupContents(ab)
		...
		self.model = ab:Add("ixOutfitModel")
		...
		function self.cost:Paint(w, h)--(text, x, y, color, alignX, alignY, font, alpha)
			ix.util.DrawText(L("Cost: ".. ix.currency.Get(self.price)), w/2, h/2, color_white, 1, 1, "ixMediumFont")
		end
	end
			...
			if (outfitList) then
				for i = 0, 1 do
					for k, v in ipairs(outfitList) do
						...
						b:SetFont("ixMediumFont")
						t:AddItem(b)

						local b = t:Add("DButton")
						b:SetText((i == 0) and "s" or "t") 
						b:SetFont("ixIconsMedium")
...
vgui.Register("ixOutfit", PANEL, "EditablePanel")